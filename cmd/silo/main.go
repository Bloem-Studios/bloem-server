...
func main() {
	...
	if mode == "proxy" || mode == "transcode" {
		...
		if mode == "proxy" {
			...
			srv.SetSourceAccess(newProxySourceAccess(pool))
	...
	liveTVSvc := bloemWireServices(appCtx, &deps, pool, cfg, nodeIdentity, sharedImageCacher)
	...
	deps.OnUserProfileSessionsRevoked = bloemProfileSessionRevoker(&compatServer)
	...
	lanAdvertiser := bloemStartListeners(appCtx, cfg, public != nil, pool, brandingSvc)
