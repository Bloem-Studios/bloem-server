...
| V05 | Tuner/guide/transcoding administration / platform admin | **Present.** `/admin/livetv`: existing discovery, lineup, sources, recordings and settings. | Shared hooks now use mounted native routes. Real tuner/provider administration was not exercised by the local fixtures. | Existing Bloem UI. |
