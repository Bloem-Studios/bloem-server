...
| The `"ok"` value of `results[].status` on `POST /api/v1/sync/progress`, replaced by `updated` (the row was written) and `ignored` (the row was accepted but not written) | TV client contract wiring — the shared client contract fixes the enum to `updated` / `ignored` / `error` | `ok` answered both a landed write and a discard: a position under the min-resume floor, or a queued offline event a newer stored event beat on last-write-wins, came back exactly like a stored row. A client that flushes an offline queue reads that as "the server has it", drops the event, and the position is gone — the one failure the sync protocol exists to prevent. There is no additive fix: the value itself is the ambiguity, and a parallel field would leave the ambiguous one authoritative for every client that already reads `status`. The route, the request body, the `error` value and its messages, and the status code are unchanged. |
