...
`DELETE /api/v2/notifications/webhooks/{id}` (`deleteNotificationWebhook`) removes the exact webhook belonging to the acting profile. Authenticated profile authority and the demo guard apply. It returns bodyless `204`, including for absent or foreign-profile IDs; unavailable service returns `503` and storage failure `500`.

The existing deletion cascades stored delivery attempts. Server-generated IDs are not reused, so repeated deletion naturally converges. This does not recall a provider request already dispatched or establish completion of external work. Callback and retry behavior for other webhooks is unchanged. The settings action captures authority, sends once without authentication replay, rejects stale receipts, and invalidates only that authority's webhook list. Administrator server channels have a separate authority and operation.
