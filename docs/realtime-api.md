...
This storage foundation does not expose a socket or validate current account/session/profile authority. The future handler must validate that authority and room proof before minting, revalidate after consumption and during the connection, enforce Origin and subprotocol, and cap connection lifetime at five minutes or the earlier authority expiry. Existing room connection, transport, callback and disconnect behavior remains separate. No API mapping or native activation follows from storage availability.
