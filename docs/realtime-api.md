...
Receipts survive suggestion and room deletion. Repeating a deleted identity conflicts instead of recreating it. Receipts retain no title, note or poster URL payload and are removed on login-account deletion. There is no age-based receipt expiry; expiring them would permit identity reuse. Legacy suggestion insertion/deletion and their wire behavior are unchanged. This storage prerequisite does not register a v2 creation endpoint, migrate its web/native callers, or claim durable broadcast delivery; those remain separate work.
