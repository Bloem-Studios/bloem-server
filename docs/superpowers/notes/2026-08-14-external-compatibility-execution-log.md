...
### 2026-08-14 — Tasks 2 through 6: parallel execution, first review wave, and fix rounds

- By maintainer decision the remaining foundation tasks ran as parallel agents in separate worktrees, and after the Codex budget was exhausted the independent-review role moved to fresh Claude reviewer agents. The structure of the gate is unchanged: implementer and reviewer are different contexts, findings are verified before implementation, fixes are RED-proven.
- Landed and merged (head `863111aa`): Task 2 (`0aa2a201` + fix `91b7fe81`), Task 3 (`49202e61` + fix `041395b1`), Task 4 (`6009eaba` + adapter `308efc9e` + seam fix `88faa402`), Task 5 (`73dab7ad` + fix `487c5c2d`), Task 6 (`b18cccae` + fix `a5f9a1ef`).
- First review wave verdicts and the fixes that answered them:
  - Task 2 rejected: the contract suites tested hand-written reference listeners — a closed loop — and the login credential was disconnected from the privileges tested. The fix binds the suites to three production-shaped routers over public HTTP on disposable databases, chains the real login's token into every privilege-denial case with semantic field assertions, repairs timing validation (interleaved, per-sample semantics, statistical tolerance), makes every violation mutation exactly single-case, and replaces websocket silence with adult-content filtering under legitimate traffic.
  - Task 3 one fix from acceptance: the test helper ignored SILO_REQUIRE_TEST_DATABASE=1, so the plan's own gates could pass on zero executed tests; fixed with a loud failure, plus atomic enrollment+audit.
  - Task 4 rejected on the seam: the capability vocabularies of the service and the contract did not intersect, so an enrolled companion could reach nothing — proven live, invisible to every test because none crossed the seam. Ruling recorded: the contract's seven slugs are canonical. The fix unifies the vocabulary with a lockstep test, maps every service sentinel to a contract error, renews by middleware-authenticated identity (mTLS renewal worked for the first time), plumbs peer TLS and image digest through enrollment, makes health honest (validated status parameter, contact-only default), adds real-service integration tests, closes idempotency mutation gaps, scopes enrollment idempotency by secret, and refuses contradictory kind declarations.
  - Task 5's package accepted, integration rejected: the gateway was mounted on the chi router, which production only reaches for /api/ paths — its families would have served the SPA forever, and the mandatory overlap tests walked the wrong tree. The fix claims the families at the public mux ahead of the SPA fallback (one canonical origin, per the design), with a composed-mux reachability suite; ownership became exact-case because case-insensitive matching would have swallowed native SPA routes. Also fixed: a backslash-redirect same-origin escape and an operator command naming a volume that does not exist.
  - Task 6 accepted with follow-up: four proven structural-scan evasions (host devices, namespace escapes, external-volume aliasing, keyword-form DSNs) now each have a check and a tamper case; 21 tamper cases total.
- Owed and tracked: the admin surface's revision-guard contract needs backing in compatapp (revision column and migration, ListApplications, revision-guarded mutations, instance-id keying) plus the production wiring of the admin service adapter and gateway StateProvider.
...
### 2026-08-14 — Tasks 3 and 4 ACCEPTED; Task 5 sent back on path ownership

- Tasks 3 (`041395b1`) and 4 (`88faa402`) are accepted. The re-review verified them by mutation rather than by reading: renaming one capability, or having the adapter quietly re-suffix projected capabilities, fails both the lockstep test and the real-service integration suite — the seam that once hid a completely inoperative feature is now genuinely crossed by tests. All sixteen service sentinels were confirmed mapped with none reaching the opaque default, the contradictory-kind refusal was confirmed not to consume the secret, and the enrollment idempotency scope was confirmed not to be a disclosure channel because colliding with it requires the victim's secret.
- Task 5 is accepted only once its path ownership is fixed, and the fault is a maintainer ruling rather than the implementer's work: ratifying exact-case ownership traded one real collision for a much larger loss. The listener being replaced canonicalizes case-insensitively and strips a leading `/emby` or `/jellyfin`; under exact case, `/system/info`, `/emby/System/Info`, and every lowercase or legacy-prefixed client path now reach the single-page app instead of the companion. Only three lowercase segments actually collide with native routes. The ruling is corrected: case-insensitive family matching returns, gated by a reserved native-segment set checked first and pinned against the frontend's own route list.
- Two structural gaps recorded with it: the production composition is still unpinned, since reverting the mux line in the entrypoint leaves every suite green — the blind spot moved with the wiring rather than closing — and the original chi-router mount survives as dead code while four mandatory route-overlap tests continue to adjudicate that dead tree.
...
### 2026-08-14 — The fourth pass, and the harness that proved nothing

- The acceptance review rejected both remaining tasks again, and its first finding is the most important single result of this campaign: the deployment scan's tamper harness was vacuous on the machine every round had been verified on. The secret-path check compared an unnormalized directory string against the normalized path the renderer emits, so the committed contract failed six checks when scanned from the repository root — and because the temporary directory on this platform ends in a separator, every tamper case was passing because the untampered contract already failed. Any non-zero exit counted as detection, so all thirty cases would have reported success with their attacks removed. The checks themselves were real; the evidence for them was not.
- The lesson is now general enough to state as a rule: a test that accepts any failure proves only that something failed. Assert the specific failure you expect, and include a control that must pass.
- The gateway's composition pin was defeated three ways, each compiling cleanly, including the two the previous round claimed to have closed — because a substring match cannot express "this expression is the only source of the listener". Replaced with an extraction: the listener is built in one function, the entrypoint holds no composition of its own, and a syntax-tree guard requires exactly one call. A build-clean edit that serves a hand-built listener and discards the gateway now fails by name.
