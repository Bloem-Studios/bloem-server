# Bloem Full Plugin Forks Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create the complete private Bloem catalog/plugin repository family, import every legally redistributable pinned upstream snapshot with clean Bloem history, and leave explicit permission gates for the two unlicensed upstreams.

**Architecture:** Each target is an independent private repository with a zero-parent Bloem root, a fetch-only upstream remote, preserved upstream license/provenance, Bloem build identity, and unchanged Silo v1 wire/storage identities. All imported Go plugins pin the immutable private SDK release `github.com/Bloem-Studios/bloem-plugin-sdk v0.13.3` without `replace`. Catalog publication and plugin releases remain a later phase; this plan establishes reviewed, testable source forks without making anything public.
...
- Change only repository/module/documentation/presentation ownership to Bloem.
...
- Do not copy Ebook or Autoscan ARR source until the copyright holder supplies a redistribution license or written permission. Their Bloem repositories are private permission-gate repositories only in this plan.
- Do not copy tracked TMDB/TVDB provider credentials into Bloem history. Their first Bloem roots must use host-supplied secret configuration and tests must prove missing credentials fail safely.
- Use `BLOEM_MODULES_TOKEN` only at CI boundaries to read the private SDK; never commit a credential-bearing URL or print the token.
...
### Task 1: Create and Import the Private Bloem Catalog

**Files:**
- Create repository: `${WORKSPACE_ROOT}/bloem-plugins`
...
- Produces: private repository `Bloem-Studios/bloem-plugins`, module `github.com/Bloem-Studios/bloem-plugins`, and allowlisted updater support for the six licensed Bloem plugin repositories.

- [ ] **Step 1: Write failing identity and updater-security tests**

Add tests which require the Bloem module/import path, reject a repository outside this literal allowlist, and reject duplicate/unsupported release assets:
...
Run `GOWORK=off go test ./...`; expect failure because Bloem identity and allowlist validation do not exist.
...
- [ ] **Step 3: Implement Bloem identity and fail-closed updating**

Set the module/import path to `github.com/Bloem-Studios/bloem-plugins`, pin SDK `v0.13.3`, make the updater accept only the literal allowlist, require a published non-draft/non-prerelease release, reject duplicate platform binaries, and require every advertised platform asset plus `checksums.txt`.

- [ ] **Step 4: Make automation private-only**

Add CI for test/vet/build and change the updater workflow to use `BLOEM_CATALOG_PUSH_TOKEN` and `BLOEM_CATALOG_SOURCE_TOKEN`, run tests before mutation, and contain no Silo dispatch endpoint or visibility/publication action.
...
Commit and push `main`; confirm `gh repo view Bloem-Studios/bloem-plugins --json visibility` returns `PRIVATE`.

### Task 2: Import MetaDB with Bloem Build Identity

**Files:**
- Create repository: `${WORKSPACE_ROOT}/bloem-plugin-metadb`
...
- Produces: private module `github.com/Bloem-Studios/bloem-plugin-metadb` with plugin ID `silo.metadb` and `metadata_provider.v1/metadb` unchanged.

- [ ] **Step 1: Add failing identity/manifest tests**

Require `silo.metadb`, API `v1`, the existing media priorities, no `replace`, the Bloem module path, and a complete Bloem catalog presentation block whose source URL is `https://github.com/Bloem-Studios/bloem-plugin-metadb`.
...
**Files:**
- Create repository: `${WORKSPACE_ROOT}/bloem-plugin-tmdb`
...
- Produces: private module `github.com/Bloem-Studios/bloem-plugin-tmdb`; unchanged plugin ID `silo.tmdb`, metadata provider `tmdb`, image scheme `tmdb://`, and host-supplied `api_key` global secret.
...
Rewrite Bloem module/source/publisher URLs, pin SDK `v0.13.3`, retain TMDB attribution/non-endorsement language, use tag-only private releases, remove Silo catalog dispatch, and require exact-tag tests before later release creation.
...
**Files:**
- Create repository: `${WORKSPACE_ROOT}/bloem-plugin-tvdb`
- Create: `NOTICE`, `scripts/verify-private-release.sh`
...
- Produces: private module `github.com/Bloem-Studios/bloem-plugin-tvdb`; unchanged plugin ID `silo.tvdb`, provider `tvdb`, image scheme `tvdb://`, and host-supplied `api_key` global secret.
...
Apply the Bloem identity/private-release rules from Task 3, preserve TVDB provider attribution, run all upstream and new tests, and confirm the target remains private.

### Task 5: Import Licensed Audiobook and Manga Plugins

**Files:**
- Create repositories: `${WORKSPACE_ROOT}/bloem-plugin-audiobook-metadata`, `${WORKSPACE_ROOT}/bloem-plugin-manga-metadata`
...
- Produces: private Bloem modules preserving `silo.audiobook-metadata` / `audiobook-metadata` and `silo.manga-metadata` / `manga-metadata`.

- [ ] **Step 1: Add failing identity/provenance tests in each repository**

Require exact compatibility IDs/config keys/priorities, exact Bloem module and presentation URLs, no `replace`, preserved AGPL license hash, and no Silo release dispatch.

- [ ] **Step 2: Import exact licensed snapshots into clean roots**

Preserve fixture attribution and provider terms, add upstream notices, rewrite only build/project identities, pin SDK `v0.13.3`, and change MangaDex's user-agent repository URL to Bloem.
...
**Files:**
- Create repositories: `${WORKSPACE_ROOT}/bloem-plugin-ebook-metadata`, `${WORKSPACE_ROOT}/bloem-plugin-autoscan-arr`
...
- Produces: private reserved Bloem repository names with no copied upstream source and a machine-readable/documented permission gate.
...
**Files:**
- Create: `${WORKSPACE_ROOT}/bloem-server/docs/plugin-fork-inventory.md`
...
Search current files and reachable Bloem history for the known upstream TMDB/TVDB credentials, credential-bearing URLs, `replace`, Silo dispatch tokens/endpoints, public-registry publication, and visibility mutation. Expected: no findings except factual upstream URLs in notices.

- [ ] **Step 3: Record the inventory and commit**

Document exact Bloem HEADs, upstream pins, license identifiers, tests, privacy evidence, and the fact that private GitHub catalog/releases cannot yet be consumed anonymously by the current server. State that the next phase is private releases plus a local/authenticated staging delivery path and the core movie/series scan acceptance suite.
