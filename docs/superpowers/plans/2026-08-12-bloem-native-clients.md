# Bloem Native Clients Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Import the complete Apple and Android clients into independent, private Bloem repositories with new application identities and compatibility with both Bloem and official Silo servers.

**Architecture:** Each client is archived from an exact upstream commit into a clean-root Bloem repository. Legal/provenance, product/package identity, brand assets, automation, and regression guards are corrected before the first push; wire/storage contracts are retained through an explicit compatibility allowlist.
...
- Repositories are `Bloem-Studios/bloem-apple` and `Bloem-Studios/bloem-android`, PRIVATE with no tags/releases.
...
- Use new application identity root `media.bloem.app` and source namespace root `media.bloem`.
...
**Files:**
- Repository: `${WORKSPACE_ROOT}/bloem-apple`
...
- Produces: private Bloem iOS/tvOS/macOS source root with independent IDs and official-server compatibility.

- [ ] Add executable policy tests that fail until the exact pin, byte-identical AGPL, complete-tree accounting, `media.bloem.app` IDs, Bloem display/build identity, compatibility allowlist, private-only workflows, and absence of Silo brand/store/signing credentials hold.
...
- [ ] Add `NOTICE`, retain third-party notices/licenses, replace Silo-owned visual assets with neutral Bloem development assets, and classify every excluded/modified upstream file.
- [ ] Rewrite project/scheme/target/archive/display identity, bundle/extension/keychain/app-group/URL identifiers and application-only symbols to Bloem; retain allowlisted protocol/storage strings.
...
- [ ] Create `Bloem-Studios/bloem-apple` PRIVATE, verify visibility before first push, configure fetch-only upstream, commit one clean root, push, and require exact-head CI green.

### Task 2: Full Android Client Import

**Files:**
- Repository: `${WORKSPACE_ROOT}/bloem-android`
...
**Interfaces:**
- Consumes: exact upstream archive and complete tree manifest.
- Produces: private Bloem phone/Android TV source root with shared independent application ID and official-server compatibility.

- [ ] Add executable policy tests that fail until the exact pin, byte-identical AGPL, complete-tree accounting, `media.bloem.app` application ID, `media.bloem` namespaces, Bloem user/build identity, compatibility allowlist, private-only workflows, and absence of Silo brand/store/signing credentials hold.
...
- [ ] Add `NOTICE`, retain third-party notices/licenses, replace Silo-owned visual assets with neutral Bloem development assets, and classify every excluded/modified upstream file.
- [ ] Mechanically relocate Kotlin/Java packages and rewrite imports/namespaces/manifests/services/providers/deep links to Bloem; retain allowlisted wire/storage strings and paired phone/TV version-code behavior.
...
- [ ] Create `Bloem-Studios/bloem-android` PRIVATE, verify visibility before first push, configure fetch-only upstream, commit one clean root, push, and require exact-head CI green.

### Task 3: Cross-Client Verification and Inventory

**Files:**
- Modify: `${WORKSPACE_ROOT}/bloem-server/docs/client-fork-inventory.md`
...
- [ ] Run protocol/API endpoint compatibility checks against Bloem Server contracts and document intentionally retained Silo wire/storage identifiers.
