# Bloem Prairie Live TV and DVR Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Selectively port Prairie Server's complete Live TV subsystem into Bloem Server and expose versioned contracts for original Bloem clients on every target.

**Architecture:** The exact Prairie Live TV slice at commit `095ecd22fbea3384a905eb9049386015db3ff4d8` is inventoried and adapted unit-by-unit to Bloem's current stores, routes, policy, playback and web systems. HDHomeRun hardware and Dispatcharr's HDHomeRun interface feed one normalized tuner model; Bloem owns guide, live sessions, timeshift and DVR. Native clients consume invented Bloem contracts and never use Prairie/Silo client code.
...
- Port only the Live TV slice and minimum Bloem integration points; never merge Prairie's unrelated 412 commits.
...
- No Prairie or Silo native-client code, assets, layouts or tests enter `bloem-apple` or `bloem-android`.
- Existing non-Live-TV Bloem behavior and public defaults remain unchanged.
...
- [ ] Write a failing test that fetches the pinned Prairie tree metadata and requires every classified path to have the exact source blob SHA, one classification (`imported`, `adapted`, `omitted`, `bloem-created`) and a nonempty rationale for omissions/adaptations.
...
- [ ] Add RED migration/store tests for a fresh database and upgrade from current Bloem HEAD, including unique tuner identity, encrypted guide secret envelope, fenced session heartbeat, profile-owned DVR rules and atomic last-good guide swap.
...
- [ ] Adapt Prairie migrations into Bloem's sequence without renumbering existing files; add forward-only preservation rules for recordings and last-good guide data.
- [ ] Port/adapt types and store queries to Bloem database conventions, secret envelopes and profile identity.
...
- [ ] Port provider clients and matching logic, routing every request through Task 3's guarded client and every secret through Bloem's encrypted store.
...
- Modify: Bloem playback/transcode dependency wiring
...
- [ ] Port/adapt Prairie stream planning, HLS bridge, codec configuration and session reclamation to Bloem playback infrastructure.
...
- [ ] Port/adapt handlers with Bloem auth/profile/policy middleware and strict request bounds.
...
**Interfaces:**
- Produces Bloem web tuner/guide/DVR administration and user guide/player/recordings surfaces.
...
- [ ] Port Prairie web behavior while adapting visual components and API hooks to current Bloem conventions; preserve Prairie attribution for imported/adapted files.
...
- [ ] Derive schemas from Task 7 Bloem handlers, not Prairie/Silo client models; specify bounds, optional fields and unknown-field behavior.
...
- [ ] Run contracts tests, disposable Bloem conformance, official capability-negative baseline and originality audits; commit/push contracts/server.
...
- [ ] Update inventory with Prairie commit, Bloem commits, CI/test evidence and explicit no-native-client-code statement; commit/push and require exact-head CI green.
