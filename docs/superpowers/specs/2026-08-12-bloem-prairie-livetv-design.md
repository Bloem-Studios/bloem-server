# Bloem Prairie Live TV and DVR Design
...
Add a complete server-native Live TV and DVR subsystem to Bloem by selectively porting Prairie Server's AGPL-3.0 implementation. Bloem will support native HDHomeRun/OTA tuners and Dispatcharr's HDHomeRun-compatible interface for IPTV provider management. It will own channel lineups, guide synchronization, live playback, transcoding, timeshift, tuner arbitration, DVR scheduling, recordings and client contracts.

Every new native Bloem client target supports Live TV at initial store release:
...
No Prairie or Silo native-client code, assets, layouts, architecture or tests enter the clean-room Bloem client repositories. Only documented Bloem Live TV contracts and invented fixtures cross the boundary.

## Licensing and provenance

Prairie Server is an AGPL-3.0 fork of Silo Server. Bloem may adapt its server-side Live TV subsystem under the license, but must preserve copyright/license notices and provide accurate attribution.

Before porting, Bloem records:

- the exact Prairie commit above;
- every imported source path and blob hash;
- every Bloem modification;
- files omitted as unrelated to Live TV;
- migration ordering and conflict resolutions;
- the final Bloem commits containing the port.

Bloem's `NOTICE` credits Prairie Server and links the exact source revision. The Prairie baseline is added as a fetch-only audit remote or a detached temporary source checkout; it is never configured as a push target.

## Port boundary

The port is limited to Prairie's Live TV slice and the minimum Bloem integration points it requires:
...
Prairie's unrelated commits and features are not merged. Each imported unit is adapted to Bloem's current architecture and tested at its boundary. Existing Bloem behavior and public defaults remain unchanged unless this specification explicitly requires a Live TV capability or operator setting.

## Provider boundary

Bloem does not directly manage arbitrary M3U, Xtream or IPTV-provider credentials in this phase.
...
- Dispatcharr exposes its HDHomeRun-compatible discovery, lineup and stream endpoints to Bloem.
- Bloem treats Dispatcharr as a tuner and owns downstream guide matching, playback sessions, timeshift and DVR.
- Dispatcharr and tuner endpoints are expected to be network-private and protected with network ACLs.

This keeps high-risk provider credentials and provider-specific behavior outside Bloem while presenting one normalized tuner model to the server and clients.
...
Clients must capability-probe before presenting a Live TV interface. Bloem enables the capability only when the subsystem is available. Capability-compatible official Silo servers work normally; servers without Live TV simply hide the feature without errors or dead navigation.

## Tuner discovery and onboarding

### LAN discovery

Bloem broadcasts SiliconDust discovery packets over UDP port `65001` and listens for HDHomeRun-compatible replies. Dispatcharr instances that expose HDHomeRun UDP discovery may appear alongside hardware tuners.

LAN discovery depends on the host network stack. Linux container deployments use an explicit Live TV Compose override with host networking. Docker Desktop cannot reliably bridge host-network UDP discovery to the physical LAN, so operators use URL probing or run Bloem directly on the host.

### URL probing

Manual onboarding accepts one HTTP(S) base URL. Bloem probes the Prairie-compatible locations:
...
Tuner channels are normalized into stable Bloem channel identities while preserving source tuner/channel identifiers for refresh and failover.
...
Guide credentials are encrypted at rest with Bloem's existing secret envelope system and never returned after write. Matching uses normalized callsigns, channel numbers and source identities, with administrator overrides for ambiguous results.
...
2. Bloem evaluates profile policy, channel access and concurrent-stream limits;
3. the scheduler chooses an available tuner and acquires a fenced lease;
4. Bloem selects direct stream, HLS bridge or transcode based on tuner capabilities and client playback capabilities;
...
- a Bloem HLS bridge that keeps protected tuner URLs server-side;
- a Bloem transcode plan using existing playback/transcode infrastructure.
...
Disk-pressure behavior is deterministic: shrink or reject new timeshift capacity before threatening finalized recordings or unrelated Bloem state.
...
6. place a successful recording into a dedicated Bloem recordings library;
...
The selective port adapts Prairie's Live TV migrations to Bloem's migration sequence. The model includes, as required by the port:
...
Migration tests cover both an empty database and upgrade from the current Bloem schema. Destructive rollback is not used for operator recordings or the last known-good guide. If a migration cannot complete safely, startup fails before partially enabling Live TV.

## API and client contracts

`bloem-client-contracts` adds versioned, invented schemas/fixtures for:
...
Fixtures use invented stations, programs and identifiers. They are derived from Bloem server behavior, not Prairie/Silo client source.
...
Bloem's web application receives the selectively ported Prairie Live TV management and user surfaces, adapted to Bloem's current design system and APIs:
...
- per-file classification: imported, adapted, omitted or Bloem-created;
...
- upgrade from current Bloem schema;
...
1. Pin and inventory the Prairie source slice; update Bloem attribution.
...
8. Port/adapt Bloem web administration, guide and live player.
...
- Bloem NOTICE/AGPL obligations are satisfied;
...
- Bloem web administration/guide/player pass;
...
- no Prairie or Silo native-client source or assets exist in Bloem client repositories.
