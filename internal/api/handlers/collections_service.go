...
func (h *CollectionHandler) CreatePersonalCollection(ctx context.Context, cmd PersonalCollectionCreateCommand) (PersonalCollectionView, error) {
	...
	if err != nil {
		if errors.Is(err, errCollectionArtworkInput) {
			return none, apiError(http.StatusBadRequest, "bad_request", "Invalid collection artwork source")
		}
		return none, apiError(http.StatusInternalServerError, "internal_error", "Failed to save collection artwork")
	}
	if posterProvided {
