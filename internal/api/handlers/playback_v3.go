...
func (h *PlaybackHandler) startPlaybackApplicationV3(r *http.Request, body []byte) (playback.DecisionResponseV3, error) {
	...
	if existing, lookupErr := h.PlanStoreV3.GetAttemptByPlaybackAttemptID(r.Context(), req.PlaybackAttemptID); lookupErr == nil {
		...
		if _, sessionErr := h.sessionMgr.GetSession(existing.SessionID); sessionErr != nil {
			return decisionResponseForFeaturesV3(playback.NewTerminalResponseV3("session_expired", "The playback session for this attempt has ended.", true), existing.NormalizedRequest.ClientFeatures), nil
		}
		return response, nil
...
func (h *PlaybackHandler) checkReplacementAdmissionV3(
	...
	// Runs even when the replacement check above already passed: transcoding
	// permission is granted per household profile, and a replacement admission
	// answers a different question (is there room for this session) than
	// whether this profile may transcode at all.
	if checker, ok := h.sessionMgr.(transcodePermissionChecker); ok &&
		(result.PlayMethod == playback.PlayTranscode || result.TranscodeAudio) {
		if err := checker.CheckTranscodingAllowed(ctx, session.UserID, session.ProfileID, result.PlayMethod == playback.PlayTranscode); err != nil {
			return sessionStartErrorV3(err)
		}
	}
	return nil
