	...
	Register(reg, op, func(ctx context.Context, in *AdminDashboardLayoutInput) (*AdminDashboardLayoutOutput, error) {
		...
		if view.UpdatedAt != nil {
			...
			if err := dec.Decode(&layout); err != nil || layout == nil {
				return nil, serviceProblem(errors.New("invalid stored dashboard layout"))
			}
			...
			return nil, serviceProblem(errors.New("stored dashboard layout lacks timestamp"))
		}
		tag := RenderETag("admin-dashboard-layout:"+strconv.Itoa(claimsFrom(ctx).UserID)+":"+profileFrom(ctx)+":"+viewerScopeDigest(ctx), revision, 1)
