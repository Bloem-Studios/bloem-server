...
type AdminNodeExecutorObservation struct {
	Incarnation ID     `json:"incarnation"`
...
type AdminNodeSession struct {
	...
	Executor          *AdminNodeExecutorObservation `json:"executor,omitempty" doc:"Observed generation, not current playback authority."`
}
...
func registerAdminNodeSessions(reg *Registry) {
	...
	Register(reg, op, func(ctx context.Context, in *AdminNodeSessionsInput) (*AdminNodeSessionsOutput, error) {
		...
		scope := CursorScope{OperationID: "listAdminNodeSessions", Security: strconv.Itoa(claimsFrom(ctx).UserID) + "/" + profileFrom(ctx), Filter: string(in.NodeID) + "/" + strconv.Itoa(in.Limit), Sort: "node,session,executor", Tiebreaker: "executor"}
		var after string
		if in.Cursor != "" {
