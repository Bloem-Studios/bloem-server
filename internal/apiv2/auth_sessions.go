...
func registerAuthSessions(reg *Registry) {
	...
	setup.Errors = []int{http.StatusConflict, http.StatusPreconditionRequired}
	Register(reg, Operation{Operation: setup, RetrySafety: RetrySafetyIdempotencyKey, DurableReplay: true, Class: ClassPublic, ServiceBacked: true, RateLimitBucket: setupDomain}, reg.setupServer)
	Register(reg, Operation{
	...
	signup.Errors = []int{http.StatusForbidden, http.StatusConflict, http.StatusPreconditionRequired}
	Register(reg, Operation{Operation: signup, RetrySafety: RetrySafetyIdempotencyKey, DurableReplay: true, Class: ClassPublic, ServiceBacked: true, RateLimitBucket: opSignup}, reg.signup)
...
func (reg *Registry) setupServer(ctx context.Context, in *SetupServerInput) (*TokenPairOutput, error) {
	...
	view, err := reg.deps.Sessions.SetupInitialUser(ctx, bloemRegistrationLifecycle(ctx, reg.registration(ctx, in.Body.Username, in.Body.Email, in.Body.Password, "", in.Body.CreateDefaultProfile, in.Body.DefaultProfileName), in.IdempotencyKey, "apiv2.auth.setup", in.RawBody))
	if err != nil {
		var apiErr *handlers.APIError
...
func (reg *Registry) signup(ctx context.Context, in *SignupInput) (*TokenPairOutput, error) {
	...
	view, err := reg.deps.Sessions.Signup(ctx, bloemRegistrationLifecycle(ctx, reg.registration(ctx, in.Body.Username, in.Body.Email, in.Body.Password, in.Body.InviteCode, in.Body.CreateDefaultProfile, in.Body.DefaultProfileName), in.IdempotencyKey, "apiv2.auth.signup", in.RawBody))
	if err != nil {
		var apiErr *handlers.APIError
