...
func TestLibraryViewsRefuseLibraryOutsideViewerScope(t *testing.T) {
	...
	h := newTestHandler(t, scopedViewerDeps(t, policy, handlers.NewSectionHandler(nil, nil), handlers.NewLibraryCollectionHandler(nil, nil, nil, 0, nil, nil, nil)))
...
func TestLibraryCollectionReadsRefuseUnknownLibraryDB(t *testing.T) {
	...
	svc := handlers.NewLibraryCollectionHandler(catalogpkg.NewLibraryCollectionRepository(pool), nil, catalogpkg.NewItemRepository(pool), 0, nil, nil, nil)
	svc.FolderRepo = catalogpkg.NewFolderRepository(pool)
