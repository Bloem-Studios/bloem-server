...
func notificationInboxFixtureCases() []fixtureCase {
	...
	cases := []fixtureCase{
		...
		{name: "notification_get_notification", operationID: "getNotification", scenario: "Profile notification inbox with synthetic records.", method: "GET", path: "/api/v2/notifications/00000000-0000-0000-0000-000000000002", headers: profileOwner(), status: 200, schema: "#/components/schemas/NotificationItem", assertHeaders: []string{"Cache-Control"}},
		{name: "notification_mark_notification_read", operationID: "markNotificationRead", scenario: "Profile notification inbox with synthetic records.", method: "POST", path: "/api/v2/notifications/00000000-0000-0000-0000-000000000002/read", headers: profileOwner(), status: 204, schema: "", assertHeaders: []string{"Cache-Control"}},
	}
	for i := range cases {
		if cases[i].schema != "" {
