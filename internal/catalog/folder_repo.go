...
func (r *FolderRepository) DeleteWithStats(
	...
	// Phase 4: release the tenancy rows that reference this folder, then delete
	// the now-lightweight folder row.
	//
	...
	// ON DELETE RESTRICT. Without this the folder delete below fails with a
	// foreign key violation and library deletion is impossible. The RESTRICT is
	// deliberately left in place as a backstop so no other path can orphan an
	// entitlement; this is the one flow that may clear them, and it does so
	// explicitly rather than by cascade. An entitlement to a library that no
	// longer exists has nothing to authorize. entitlement_audit_events carries
	// no foreign key here, so the audit trail survives the delete.
	if err := retryOnDeadlock(ctx, func() error {
		_, e := r.pool.Exec(ctx, `DELETE FROM organization_entitlements WHERE media_folder_id = $1`, id)
		return e
	}); err != nil {
		return nil, fmt.Errorf("releasing folder entitlements: %w", err)
	}

	// Tolerate 0 rows so a resumed run that already removed it still succeeds.
	if err := retryOnDeadlock(ctx, func() error {
