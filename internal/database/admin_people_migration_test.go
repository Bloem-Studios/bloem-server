...
func TestAdminPeopleMigrationsUpDownUpWithOrganizationAuditRows(t *testing.T) {
	...
	var hasTargets bool
	if err := db.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name='admin_people_selections' AND column_name='targets')`).Scan(&hasTargets); err != nil {
		t.Fatal(err)
	}
	if !hasTargets {
