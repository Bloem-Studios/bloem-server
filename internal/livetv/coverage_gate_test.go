...
func TestMediaDialPolicyBlocksResolvedLoopback(t *testing.T) {
	if !isBlockedMediaIP(net.ParseIP("127.0.0.1")) || !isBlockedMediaIP(net.ParseIP("::1")) {
		t.Fatal("resolved loopback addresses must be blocked at dial time")
	}
	if isBlockedMediaIP(net.ParseIP("192.168.1.1")) {
		t.Fatal("private tuner LAN address should remain allowed")
	}
}
