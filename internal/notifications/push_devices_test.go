...
func newPushDeviceTestRepo(t *testing.T) (*PushDeviceRepository, *pgxpool.Pool) {
	...
	migration, err := os.ReadFile("../../migrations/sql/20260906054410_ordered_android_push_registrations.sql")
	if err != nil {
		t.Fatal(err)
	}
	ddl := strings.Split(strings.Split(string(migration), "-- +goose StatementBegin")[1], "-- +goose StatementEnd")[0]
	ddl = strings.Replace(ddl, "CREATE TABLE public.android_push_installations", "CREATE TEMP TABLE android_push_installations", 1)
