...
func TestHardwareVideoDecoder(t *testing.T) {
	...
	for codec, want := range cases {
		if got := hardwareVideoDecoder(transcodeHWNVENC, codec); got != want {
	...
	for _, accel := range []string{"qsv", transcodeHWVAAPI, "none", ""} {
...
func TestResolveLiveVideoDecoderModes(t *testing.T) {
	...
	if got := resolveLiveVideoDecoder(HWDecodeAuto, transcodeHWNVENC, "mpeg2video", withDecoder); got != "mpeg2_cuvid" {
		t.Fatalf("auto with decoder = %q, want mpeg2_cuvid", got)
	}
	if got := resolveLiveVideoDecoder(HWDecodeAuto, transcodeHWNVENC, "mpeg2video", withoutDecoder); got != "" {
		t.Fatalf("auto without decoder = %q, want software", got)
	}
	if got := resolveLiveVideoDecoder(HWDecodeOff, transcodeHWNVENC, "mpeg2video", withDecoder); got != "" {
		t.Fatalf("off = %q, want software", got)
	}
	// "on" trusts the operator and skips the probe.
	if got := resolveLiveVideoDecoder(HWDecodeOn, transcodeHWNVENC, "mpeg2video", withoutDecoder); got != "mpeg2_cuvid" {
...
func TestLiveFallbackLadder(t *testing.T) {
	...
	if ladder[0].HWDecode != HWDecodeOn || ladder[0].HWAccel != transcodeHWNVENC {
		t.Fatalf("first rung = %+v, want the configured pipeline", ladder[0])
	}
	if ladder[1].HWDecode != HWDecodeOff || ladder[1].HWAccel != transcodeHWNVENC {
	...
	copySession := LiveHLSOpts{HWAccel: transcodeHWNVENC}
