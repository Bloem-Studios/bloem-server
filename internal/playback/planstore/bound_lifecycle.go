...
// EnsureAdmittedPlaybackSource applies the retained first-admission protocol.
// Existing registrations, including blocked and retiring ones, remain authoritative.
func (s *Postgres) EnsureAdmittedPlaybackSource(ctx context.Context, accountID int) (playback.AdmittedPlaybackSourceV3, error) {
	decision, err := pgstore.NewPostgresProvider(s.db).AutomaticFirstAdmission(ctx, accountID)
	if err != nil {
		return playback.AdmittedPlaybackSourceV3{}, err
	}
	return playback.AdmittedPlaybackSourceV3{Source: userstore.PlaybackSourceRef{Backend: userstore.PlaybackSourcePostgres, AccountID: accountID, SourceID: decision.Intent.SourceID, SelectionGeneration: 1}, AdmissionID: decision.Intent.IntentID}, nil
}
