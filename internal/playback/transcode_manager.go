...
type TranscodeManager struct {
	...
	// StrictAdmissionFn is Bloem's strict reconstruct admission hook; see
	// bloem_transcode_manager.go. Nil keeps upstream behavior.
...
func (m *TranscodeManager) reconstructSession(ctx context.Context, sessionID string, requestUserID int, card RecipeCard, deferToneMapConfirmation bool) (*Session, *Session, bool) {
	...
	if err != nil {
		...
		if m.refuseUnevaluatedReconstruct(ctx, sessionID, card.UserID, method, err) {
			return nil, nil, false
		}
