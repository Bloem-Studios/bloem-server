...
type ebookSkipFile struct {
	...
	groupKeyVersion int
	status          string
	coverRetry      bool // Bloem: a previous scan failed to upload the local cover
}
...
func (r *FileRepository) loadEbookSkipState(ctx context.Context, folderID int, paths []string) (ebookSkipState, error) {
  ...
  SELECT mf.observed_root_path, COALESCE(mf.content_id, ''), mf.file_path, mf.file_size,
         ...
         EXISTS (SELECT 1 FROM ebook_cover_retries r WHERE r.media_file_id = mf.id)
  FROM media_files mf
 ...
 `, folderID, paths)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&root, &file.contentID, &file.path, &file.size, &file.modifiedAt, &file.groupKeyVersion, &file.status, &file.coverRetry); err != nil {
