...
func ProtocolSegmentAcknowledgement() workerprotocol.Operation {
	...
	parameters := []*huma.Param{
		{Name: sessionParameter, In: pathParameter, Required: true, Schema: &huma.Schema{Type: huma.TypeString}, Description: "Existing transport session; not authority for a replacement executor."},
		...
		{Name: "X-Silo-Stream-Token", In: headerParameter, Schema: &huma.Schema{Type: huma.TypeString}, Description: "Signed executor namespace reference when the session is bound. Missing/mismatched bound references fail closed."},
	}
	...
	return workerprotocol.Operation{Listener: listener, Method: http.MethodPost, Path: "/transcode/{session_id}/segment/{name}/downloaded", Handler: "(*internal/transcodenode.Server).handleSegmentDownloaded", AuthClass: bearerClass, Parameters: parameters, Responses: responses, RetrySafety: idempotent,
		Description: "Record downstream completion only for the exact session incarnation and timeline, after executor namespace and serving-grant checks. A stale nonempty generation is ignored with 204; no reconstruction, durable receipt or client-visible generation is implied."}
}
