...
func ProtocolTranscodeStart(schemas huma.Registry) workerprotocol.Operation {
	...
	op := workerprotocol.Operation{
		...
		Description: "Start a process-local HLS session after input, recipe, GPU and configuration guards. Ordinary JSON decoding ignores unknown keys and defaults omitted fields; there is no decoder body limit. Bound starts require an authoritative immutable recipe matching every submitted option and the full recipe digest, the selected node, execution grants and RequireReady. Their response echoes the exact namespace and recipe digest after readiness; no hardware retry occurs. Existing bound sessions cannot be replaced even by the same namespace. Legacy same-ID starts can tear down and replace an earlier session before failing, and legacy RequireReady may retry early hardware startup failure in software. Monitoring runs asynchronously. No durable admission/replay receipt or automatic caller retry is implied.",
