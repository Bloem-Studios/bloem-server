...
func ProtocolTranscodeStart(schemas huma.Registry) workerprotocol.Operation {
	...
	op := workerprotocol.Operation{
		...
		Description: "Start a process-local HLS session after input, recipe, GPU and configuration guards. Ordinary JSON decoding ignores unknown keys and defaults omitted fields; there is no decoder body limit. Bound executor execution requires its grant authority; existing bound sessions cannot be replaced even by the same namespace. Legacy same-ID starts can tear down and replace an earlier session before failing. RequireReady waits for a manifest and may retry early hardware startup failure in software; otherwise 202 does not establish playable media. Monitoring runs asynchronously. No durable admission/replay receipt or automatic caller retry is implied.",
