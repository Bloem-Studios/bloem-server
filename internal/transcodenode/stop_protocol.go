...
// It deliberately does not advertise stop authority for bound executors.
func ProtocolLegacyStop() workerprotocol.Operation {
	...
	op := workerprotocol.Operation{
		...
		Description: "Stop a legacy transport under its lifecycle lock. Bound in-memory or stored executor namespaces are refused with 409; node bearer alone is not executor stop authority. Progressive cancellation installs a process-local token-lifetime fence; stored recipe deletion is required when configured. Local teardown may precede a 503 authority-deletion or shutdown failure. A missing session/recipe returns 404 after cleanup attempts. This ID-addressed command has no durable replay identity and must not be automatically repeated against a possible successor.",
		Parameters:  []*huma.Param{{Name: sessionParameter, In: pathParameter, Required: true, Schema: &huma.Schema{Type: huma.TypeString}, Description: "Legacy transport identifier; not a bound executor namespace or durable stop receipt."}},
	...
	for _, status := range []int{401, 404, 409, 503} {
