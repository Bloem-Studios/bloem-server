...
func ProtocolStreaming() []workerprotocol.Operation {
	...
	for _, mount := range mounts {
		for _, method := range mount.methods {
			...
			token := &huma.Param{Name: streamToken, In: headerParameter, Schema: &huma.Schema{Type: huma.TypeString}, Description: "Signed recipe/executor reference for node-side validation; not a native user access token."}
			...
			statuses := []int{401, 404, 409, 422, 503}
			switch mount.kind {
			case kindRemux:
				token.Required = true
				statuses = []int{400, 401, 404, 409, 410, 500, 503}
				op.Description = "Node bearer plus independently verified playback recipe token must bind exact transport and this node's remux-execution/proxy-egress route. Bound executor recipes return409 because progressive executor authority is not configured. Audio-downmix recipes require the exact stereo shape; input authority and current configuration are checked. Revoked/stopped authority returns410 and a concurrent GET returns409. GET starts request-scoped MP4 remux with cancellation/stop/reload tracking; committed200 can truncate on encoder/read failure. HEAD validates authority but does not check actual file existence or start an encoder. Process-local fences are not durable across node replacement."
			...
			case kindManifest, kindSegment:
				op.Description = "Node bearer protects the worker listener. Existing bound sessions require the exact signed executor namespace and current serving grant. Missing process-local sessions may reconstruct through existing signed-token/stored-recipe, input and execution authority guards; a missing/refused reconstruction can return404. Serving-grant failure returns503. Reads refresh liveness; no durable session availability, unscoped reconstruction or native route is promised. "
				if mount.kind == kindManifest {
					...
					op.Description += "Build the playback manifest under its current serving grant; unavailable manifest returns503. No-store response contains relative segment links."
