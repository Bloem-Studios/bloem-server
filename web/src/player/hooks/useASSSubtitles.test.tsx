...
it("fetches ASS and attached fonts with captured headers and no URL credentials", async () => {
  const headers = Object.freeze({ authorization: "Bearer captured", "x-profile-id": "profile" });
  const track = {
    ...attachedFontTrack,
    url: "https://proxy.example.test/stream/v3/session/subtitles/8.ass?file_id=1&embedded_stream_index=8",
    font_bundle_url:
      "https://proxy.example.test/stream/v3/session/subtitles/8/fonts?file_id=1&embedded_stream_index=8",
    request_headers: headers,
    font_request_headers: headers,
  ...
  const fetch = vi
    ...
    .mockImplementation((url: string) =>
      Promise.resolve(
        url === track.font_bundle_url
          ? mockFontBundleResponse("font")
          : mockFetchResponse("[Script Info]"),
      ),
    );
  vi.stubGlobal("fetch", fetch);
  const first = renderHook(() => useASSSubtitles(makeVideoRef(), [track], 8, false, 0, 0));
  await waitFor(() => expect(constructorOpts).toHaveLength(1));
  for (const url of [track.url, track.font_bundle_url])
    expect(fetch).toHaveBeenCalledWith(
      url,
      expect.objectContaining({ headers, redirect: "error", credentials: "omit" }),
    );
  first.unmount();
  const replacement = Object.freeze({ authorization: "Bearer second", "x-profile-id": "second" });
  renderHook(() =>
    useASSSubtitles(
      makeVideoRef(),
      [{ ...track, request_headers: replacement, font_request_headers: replacement }],
      8,
      false,
      0,
      0,
  ...
  expect(fetch).toHaveBeenCalledWith(
    ...
    expect.objectContaining({ headers: replacement }),
  );
  expect(fetch.mock.calls.filter(([url]) => url === track.font_bundle_url)).toHaveLength(2);
});
