...
it("loads VTT through the captured header fetch and programmatic track", async () => {
  const headers = Object.freeze({ authorization: "Bearer captured", "x-profile-id": "profile" });
  const track = {
    ...srtTrack,
    url: "https://proxy.example.test/stream/v3/session/subtitles/1.vtt?file_id=1&embedded_stream_index=1",
  ...
  renderHook(() => useSubtitleTracks(video, [track], 1, 0, 0, { current: 100 }, { current: 0 }));
  await waitFor(() => expect(createdTracks[0]?.cues).toHaveLength(1));
  expect(fetchMock).toHaveBeenCalledWith(
    `${track.url}&position=0&duration=600`,
    expect.objectContaining({ headers, redirect: "error", credentials: "omit" }),
  );
  expect(video.current?.querySelectorAll("track")).toHaveLength(0);
});

it("discards an in-flight VTT response after captured authority changes", async () => {
  let current = true;
  let deliver!: (response: ReturnType<typeof vttResponse>) => void;
  ...
  const track = {
    ...srtTrack,
    request_headers: { authorization: "Bearer captured", "x-profile-id": "profile" },
    request_is_current: () => current,
  };
  const videoRef = makeVideoRef();
  renderHook(() => useSubtitleTracks(videoRef, [track], 1, 0, 0, { current: 100 }, { current: 0 }));
  await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1));
  const signal = (fetchMock.mock.calls[0]![1] as RequestInit).signal!;
  await act(async () => {
    current = false;
    deliver(vttResponse("WEBVTT\n\n00:00:01.000 --> 00:00:02.000\nold profile\n\n"));
  });
